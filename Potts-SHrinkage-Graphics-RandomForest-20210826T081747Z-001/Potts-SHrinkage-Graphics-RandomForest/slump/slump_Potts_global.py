...
with open(r"slump_Partitions_Sets.pkl", "rb") as input_file:
...
def Make_Matrix_Semi_Sparse(X, l0,l1,l2):
    
    """
    This function make a matrix Semi Sparse
    
    X is set as input for this function. X size is: l0xl1 + l1xl2
    
    """
    
    X_recuperation = X.copy()
    
    X_temp = X.copy()
    
    i = 0
    #Let's extract the precision block between W1 and W2
    for gcolum in range(l0*l1, l0*l1+ l1):
        
        
        for j in range(0,l1*l2,l1):
            
            #Set generaly the column to zero
            X_temp[0:(l0*l1),gcolum+j] = 0
        
            #Change the precision block for lines [(l0*i):(l0*(i+1))]
            X_temp[(l0*i):(l0*(i+1)),gcolum+j] = X[(l0*i):(l0*(i+1)),gcolum+j]    
        
        #We move in lines changes
        
        i = i+1
        
    #Now that everything is done with the precision block between W1 and W2,
    #we need to translate into its symetric part.
    
    block_to_translate = X_temp[0:(l0*l1),(l0*l1):(l0*l1+l1*l2)]
    
    X_temp[(l0*l1):(l0*l1+l1*l2),0:(l0*l1)] = np.transpose(block_to_translate)
...
def Train_My_Potts_Cluster(Cluster_xtrain,Cluster_ytrain, Cluster_xtest,Cluster_ytest):
    ...
    while (Continue_Principle == 0 ):
            ...
            try:
                ...
                my_qw_evalb = Make_Matrix_Semi_Sparse(my_qw_evalb, l_0,l_1,l_2) # This is for the fully-sparse spnnr model
